...
export function assertManagedQqProfile(content, expectedPeer, allowDisabled = false) {
  ...
  if (ids.length !== 1 || names.length !== 1 || peers.length !== 1 || peers[0][1] !== expectedPeer || (allowDisabled ? !/^\s*disabled:\s+(?:true|false)\s*$/mu.test(content) : !/^\s*disabled:\s+false\s*$/mu.test(content)) || !/^\s*c2cMode:\s+allowlist\s*$/mu.test(content) || !/^\s*groupMode:\s+disabled\s*$/mu.test(content) || /(^|\n)\s*-\s+insert:/u.test(content)) throw new Error('managed QQ profile binding mismatch');
}
...
if (process.argv[1]?.endsWith('validate-qq-profile.mjs')) {
  ...
  assertManagedQqProfile(readFileSync(process.argv[fileIndex + 1], 'utf8'), process.argv[peerIndex + 1], process.argv.includes('--allow-disabled'));
}
