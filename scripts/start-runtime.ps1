...
if ($DryRun) {
  if ($LiveQQ) { $dryArgs = @((Join-Path $repo 'packages/dsh-host/dist/cli.js'), '--live-qq'); $dryCwd = $config.workspace; $dryCommand = 'node' } else { $dryArgs = @('--filter','@personal-growth/dsh-adapter','exec','dsh','web','--patch',$disabledPatch,'--port', [string]$config.webPort); $dryCwd = $config.workspace; $dryCommand = 'corepack' }
