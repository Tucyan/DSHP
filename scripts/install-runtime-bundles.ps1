...
if (-not $PeerId -or $PeerId.Contains("`n") -or $PeerId.Contains("`r") -or $PeerId.Contains(':') -or $PeerId.Contains('[') -or $PeerId.Contains(']')) { throw 'A single QQ peer id is required with -PeerId; it is written only to the isolated profile allowlist.' }
...
try {
  ...
  if ($LASTEXITCODE -ne 0) { throw "DSH bundle installation failed with exit code $LASTEXITCODE" }
...
- insert:
    - id: im-qqbot
      ...
      config:
        ...
        debug: false
"@
  Set-Content -LiteralPath $profilePatch -Value $patchText -Encoding utf8
