...
let modelSelection = { provider: 'deepseek-official', model: 'deepseek-v4.1-flash-expires-on-0910' }; let modelRevision = 0
const models = { view: () => ({ selection: { ...modelSelection }, revision: modelRevision, configPath: join(root, 'runtime/dsh-home/settings.yaml'), applies: 'live' }), async update(selection, expectedRevision) { if (expectedRevision !== modelRevision) throw new Error('model_conflict'); modelSelection = { ...selection }; modelRevision += 1; return this.view() } }
const backend = new AdminBackend({ files, memory, prompts, heartbeat, status, sessionIds: ['ui-test-foreground', 'ui-test-foreground-hidden-dream'], sessions: { async list() { return [{ id: 'ui-test-foreground' }, { id: 'ui-test-foreground-hidden-dream' }] }, async read(id, from) { return { id, events: events.filter(event => event.seq >= from) } } }, schedule: async operation => { if (operation !== 'list') throw new Error('fixture has no schedule runtime'); return [] }, pending: async () => ({ memory: 0, outbound: 0 }), models, internalPrompts: { decision: '测试环境：仅返回 NOOP。' } })
