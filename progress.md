...
- Pushed `edc8ed9` and corrective `cfed6c1`; deployed/built `cfed6c1`. Authenticated production update wrote revision 1 to `/opt/dshp/runtime/dsh-home/settings.yaml`; status is running/QQ ready with empty queues. Listener is loopback-only, unauthenticated model API is 401, settings mode is 0600 root:root, Nginx is active, and restart count stabilized at the six pre-fix failures.
