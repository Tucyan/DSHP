...
describe('expanded acceptance cases', () => {
  ...
  it('MERGE archives the source and updates the target', async () => { const service = new MemoryService({ workspace: await root() }); await service.apply(ProposalSchema.parse({ action: 'CREATE', path: 'preferences/source.md', summary: 'Source', content: 'source', sourceEvidence: ['s'] })); const target = await service.apply(ProposalSchema.parse({ action: 'CREATE', path: 'preferences/target.md', summary: 'Target', content: 'target', sourceEvidence: ['t'] })); const source = await service.read('preferences/source.md'); await service.apply(ProposalSchema.parse({ action: 'MERGE', path: 'preferences/source.md', targetPath: 'preferences/target.md', summary: 'Merged', content: 'merged', sourceEvidence: ['m'], expectedHash: source.hash })); expect((await service.read('preferences/target.md')).content).toBe('merged'); expect((await service.read('archive/source.md')).metadata.sources).toEqual(['s', 'm']); expect(target.accepted).toBe(true); });

  it('recovers an archive pending journal without duplicate revision', async () => { const workspace = await root(); const service = new MemoryService({ workspace }); const created = await service.apply(ProposalSchema.parse({ action: 'CREATE', path: 'preferences/recover.md', summary: 'Recover', content: 'body', sourceEvidence: ['h'] })); const hash = created.revision!.afterHash!; const sourceRaw = (await service.read('preferences/recover.md')).raw; const archiveRaw = sourceRaw.replace('"category":"preferences"', '"category":"archive"'); const archiveHash = sha(archiveRaw); const pendingRevision = RevisionSchema.parse({ revisionId: 'archive-recovery', time: '2026-01-01T00:00:00Z', actor: 'test', action: 'ARCHIVE', path: 'preferences/recover.md', source: ['crash'], beforeHash: hash, afterHash: archiveHash }); await writeJsonAtomic(service.paths.state, { memoryCursor: {}, pendingMutation: { action: 'ARCHIVE', path: 'preferences/recover.md', targetPath: 'archive/recover.md', targetBeforeHash: null, source: ['crash'], beforeHash: hash, archiveHash, afterHash: archiveHash, afterRaw: archiveRaw, archiveRaw, revision: pendingRevision } }); const restarted = new MemoryService({ workspace }); await expect(restarted.read('archive/recover.md')).resolves.toBeDefined(); await restarted.apply(ProposalSchema.parse({ action: 'IGNORE', reason: 'wake again', sourceEvidence: ['test'] })); const revisions = (await readFile(service.paths.revisions, 'utf8')).trim().split(/\r?\n/).map((line) => JSON.parse(line)); expect(revisions.filter((revision) => revision.revisionId === 'archive-recovery')).toHaveLength(1); });
});
