...
export class DurableQqPort implements QqPort {
  ...
  async receive(): Promise<AgentTrigger | null> { await this.ready(); while (this.inbound.length) { const trigger = this.gate.accept(this.inbound.shift()!); if (trigger) return trigger; } return null; }
  async send(message: QqOutbound): Promise<boolean> { await this.ready(); if (message.background) throw new Error('background QQ sends are prohibited'); if (!message.idempotencyKey) throw new Error('idempotencyKey is required'); const result = await this.store!.claim(message.idempotencyKey, message.occurrenceId); if (result === 'sent') return false; try { await this.transport.sendPrivate(this.config.peerId, message.text); } catch (error) { await this.store!.fail(message.idempotencyKey); throw error; } await this.store!.complete(message.idempotencyKey); return true; }
