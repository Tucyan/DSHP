...
describe('live composition contract', () => {
  ...
  it('starts a consumable live controller when credentials and host bridge are supplied', async () => {
    ...
    try {
      const controller = await runLive(root, { peerId: 'peer-2', inbound: inbound(), transport: { sendPrivate: async (_peer, text) => { sent.push(text); } }, scheduleTool: { create: async () => ({ id: 'x' }), list: async () => [], delete: async () => true } });
