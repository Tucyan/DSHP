export interface VisibleMessage { id: string; role: 'user' | 'assistant'; text: string; at: string }
...
export function normalizeVisibleMessages(events: readonly ActivityEvent[], options: { includePluginUsers?: boolean; sessionId?: string } = {}): VisibleMessage[] {
  const includePluginUsers = options.includePluginUsers ?? true
  const sessionId = options.sessionId ?? 'session'
  const result: VisibleMessage[] = []
  const sentIds = new Set<string>()
  for (const event of events) {
    const data = (event.data ?? {}) as { source?: Source; content?: Block[]; message?: MessageLike }
    if (event.type === 'personal-growth/message-sent') {
      ...
      sentIds.add(id)
      if (Number.isFinite(event.time)) result.push({ id, role: 'assistant', text: text.slice(0, 1000), at: new Date(event.time!).toISOString() })
      continue
    }
    ...
    result.push({ id: `${sessionId}:${event.seq ?? result.length}`, role: 'user', text: text.slice(0, 1000), at: new Date(event.time!).toISOString() })
  }
  return result
}
