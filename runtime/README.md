...
QQ deployment has two gates: the package-level bundle layer is disabled by default, and the opt-in installer writes a profile-level, name-checked overlay for exactly one quoted peer before it installs the Tencent bundle. Until that overlay exists, no QQ user is reachable. Use only `install-runtime-bundles.ps1 -Install -PeerId ...`; installing the official Tencent package directly is unsupported. The installer expects dependencies to already be present and automatically runs the project-root build so every local bundle has a `dist` artifact before rendering its overlay. Non-live starts always add the checked-in disabled patch, while live starts require the managed overlay and matching `QQ_PEER_ID` (or `-PeerId`).
